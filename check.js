    ...
    let user = { name: "", avatar: "😎", gold: 500, xp: 0, level: 1, wordLevel: 1, careerLevel: 1, careerWins: 0, clan: "", winStreak: 0, extraSpins: 0, inventory: [], totalMinutes: 0, lastVaultTime: 0, claimedCareerRewards: {}, items: { oracle: 0, freeze: 0, sabotage: 0 } }; let gamesPlayedCount = 0; // Reklam sayacı
    ...
    // --- KELİME OYUNU VERİTABANI (TÜRKİYE & TARİH) ---
            const WORD_DATA = {
    ...
    // --- KELİME KARİYER AYARLARI ---
    const WORD_CAREER_LEVELS = {
        1: { length: 3, reward: 200, desc: "Kelime Çırağı (3 Harf)", icon: "🌱" },
        2: { length: 4, reward: 400, desc: "Hece Ustası (4 Harf)", icon: "🌿" },
        3: { length: 5, reward: 600, desc: "Kelime Avcısı (5 Harf)", icon: "🏹" },
        4: { length: 6, reward: 800, desc: "Sözlük Kurdu (6 Harf)", icon: "📚" },
        5: { length: 7, reward: 1200, desc: "Edebiyatçı (7 Harf)", icon: "🎓" },
        6: { length: 8, reward: 2000, desc: "Bilgin (8 Harf)", icon: "🧠" },
        7: { length: 9, reward: 3500, desc: "Filolog (9 Harf)", icon: "🏺" },
        8: { length: 10, reward: 5000, desc: "Kelimelerin Efendisi (10 Harf)", icon: "👑" }
    };
...
const THEMES = [
    ...
    const AdManager = {
        ...
        */
        ...
        // Yandex Units (ACTIVE)
        yandexRewardedId: "R-M-18543851-5", // Updated
        yandexInterstitialId: "R-M-18543851-4", // Updated
        ...
        pickRewardedUnitId: function() {
            // ONLY YANDEX
            return this.yandexRewardedId;
        },

        pickInterstitialUnitId: function() {
            // ONLY YANDEX
            return this.yandexInterstitialId;
        },
    ...
    function useItem(type) {
        if (currentScreen === 'screen-word-game') {
            ...
            if (!user.items || !user.items[type] || user.items[type] <= 0) return showToast("Eşya yok!");

            if (type === 'oracle') {
                if (wordGameState.usedItems && wordGameState.usedItems.includes('oracle')) {
                    return showToast("⚠️ İpucu her oyunda 1 kez kullanılabilir!");
                }
                if (!wordGameState.usedItems) wordGameState.usedItems = [];
                wordGameState.usedItems.push('oracle');

                // Reveal 1 letter (Green)
                // Find a letter in secret that hasn't been guessed correctly yet?
                // Or just reveal a random position?

                const secret = wordGameState.secret;
                let revealed = false;

                // Try to find unrevealed index
                for(let i=0; i<10; i++) {
                    const idx = Math.floor(Math.random() * secret.length);
                    // Check if we already know this (from board)?
                    // Simplified: Just tell the letter.
                    alert(`🔮 KAHİN: ${idx+1}. Harf: ${secret[idx]}`);
                    revealed = true;
                    break;
                }
...
// --- GOLD ANIMATION FUNCTION ---
    ...
    // --- PIGGY BANK WIN ANIMATION FUNCTION ---
        ...
        // DELAYED AD TRIGGER (To prevent White Screen)
        // Wait until UI is fully visible (2.5s)
        setTimeout(() => {
            // Trigger Native Logic if available
            if (typeof window.Android !== 'undefined') {
                try {
                    // Notify Native side about win screen (optional, if native needs to prep rewarded)
                    if (typeof window.Android.onWinScreenShown === 'function') {
                        window.Android.onWinScreenShown(amount);
                    }

                    // Try to show Interstitial after win (non-blocking if not ready)
                    // Added extra safety delay
                    if (typeof window.Android.tryShowInterstitialAfterWin === 'function') {
                        window.Android.tryShowInterstitialAfterWin();
                    }
    ...
    // --- NATIVE AD CALLBACKS (Called from Android) ---
    function onAdRewardEarned() {
        // Called when Rewarded Ad is completed
        const amountEl = document.getElementById('piggyAmount');
        const currentText = amountEl.innerText.replace('+','').replace(' Altın','');
        let currentAmount = parseInt(currentText) || 0;

        if (currentAmount > 0) {
            updateGold(currentAmount); // Add the same amount again (Double)
            amountEl.innerText = `+${currentAmount * 2} Altın`;
            amountEl.style.color = '#2ecc71';
            amountEl.style.animation = 'pulse-active 1s infinite';

            showToast("💰 Para 2'ye Katlandı!");
            AudioEngine.win();

            // Disable button
            const btnDouble = document.getElementById('btnDoubleMoney');
            if(btnDouble) {
                btnDouble.disabled = true;
                btnDouble.innerText = "✅ 2x Alındı";
                btnDouble.style.opacity = "0.6";
            }
        }
    }

    function onAdDismissed() {
        // Called when ad is closed (Interstitial or Rewarded)
        // Ensure UI is interactive
        console.log("Ad dismissed via Native Callback");
    }

    function onAdFailed() {
        showToast("Reklam yüklenemedi. Lütfen internet bağlantını kontrol et.");
    }

    function onDoubleMoneyClicked() {
        if (typeof window.Android !== 'undefined' && typeof window.Android.showDoubleMoneyAd === 'function') {
            window.Android.showDoubleMoneyAd();
        } else {
            // Fallback for Web/Test
            console.log("Native showDoubleMoneyAd not found. Simulating...");
            if(confirm("Reklam izlendi varsayılsın mı? (Web Test)")) {
                onAdRewardEarned();
            }
        }
    }
    ...
    function endGameLocal(r, amount) {
        ...
        try {
            ...
            if (gamesPlayedCount % 2 === 0) {
                ...
                // NATIVE ENTEGRASYON İÇİN KALDIRILDI
                /*
                setTimeout(() => {
                    AdManager.showInterstitialAd();
                }, 1500);
                */
                // Native tarafa sinyal gönder (Gerekirse)
                if (typeof window.Android !== 'undefined' && typeof window.Android.tryShowInterstitialAfterWin === 'function') {
                     // Burada çağırmıyoruz, çünkü zaten Win Screen açıldığında çağrılacak.
                     // Sadece log basıyoruz.
                     console.log("Game end check: Eligible for ad, waiting for Win Screen...");
                }
            }
        } catch (e) {
...
function startSingle() {
    ...
    function requestRematch() {
        // 1. WORD GAME (SINGLE/CAREER)
        if (currentScreen === 'screen-word-game' || (wordGameState && wordGameState.active)) {
            if (wordGameState.mode === 'single' || wordGameState.mode === 'pvc') {
                restartWordGame();
                changeScreen('screen-word-game');
            } else if (wordGameState.mode === 'career') {
                startWordCareerLevel(wordGameState.level);
            } else if (wordGameState.mode === 'online') {
                initiateOnlineRematch('word');
            }
            return;
        }

        // 2. NUMBER GAME (SINGLE/BOUNTY/CAREER)
        if (game.mode === 'single' || game.mode === 'pvc') {
            startSingle();
            changeScreen('screen-game');
        } else if (game.mode === 'bounty') {
            openBountyScreen(); // Bounties don't have direct rematch, go to list
        } else if (game.mode === 'career') {
            // Restore last level index
            if (game.careerLvlIdx) selectedCareerLvl = game.careerLvlIdx;
            launchCareerGame();
        } else if (game.mode === 'online') {
            initiateOnlineRematch('number');
        }
    }

    // New Function to handle Online Rematch Initiation
    function initiateOnlineRematch(type) {
        if (game.rematchRequested) return;
        game.rematchRequested = true;

        const btn = document.getElementById('btnRematch');
        if(btn) { btn.disabled = true; btn.innerText = "İstek Gönderildi..."; }

        // Send Rematch Offer to Firebase
        const path = 'rooms/' + game.room + '/rematch/' + game.role;
        db.ref(path).set({
            wants: true,
            timestamp: Date.now()
        });

        showToast("Rövanş isteği gönderildi...");
    }
    ...
    function updateGold(amount) {
        ...
        if (user.name) {
            ...
            if (finalAmount > 0) {
                recordIncomeForUser(user.name, finalAmount);
            }
        }
    ...
    function adminModifyGold(target, amount) {
        db.ref('users/' + target + '/gold').transaction(current => (current || 0) + amount).then(() => {
            showToast(`${target}: ${amount > 0 ? '+' : ''}${amount} Altın`);
            // Admin eklemelerini de sıralamaya yansıt
            if (amount > 0) recordIncomeForUser(target, amount);
        });
    }
    ...
    function listenRoom(c) {
        ...
        db.ref('rooms/' + c).on('value', s => {
            ...
            // Bağlantı kopma kontrolü (Rövanş sırasında es geç)
            if (game.active && r.status === 'playing' && (!r.p1 || !r.p2) && !game.rematchRequested) {
                game.active = false;
    ...
    function adminResetWordCareer() {
        if (!confirm("⚠️ TÜM Kelime Kariyer ilerlemelerini SIFIRLAMAK üzeresin. Onaylıyor musun?")) return;
        ...
        db.ref('users').once('value').then(snap => {
            ...
            db.ref().update(updates).then(() => {
                showToast("✅ Kelime Kariyeri Sıfırlandı!");
                user.wordLevel = 1;
                user.wordCareerWins = 0;
                user.claimedWordRewards = {};
                updateLobbyUI();
...
<div style="text-align:left; padding:10px; line-height:1.6;">
    <h3 style="color:var(--primary); border-bottom:2px solid var(--primary); padding-bottom:5px;">🎮 OYUN ÖZELLİKLERİ</h3>

    <h4 style="color:#e67e22; margin-top:15px;">🔢 Sayı Tahmin (Klasik)</h4>
    <ul style="padding-left:20px; font-size:0.9rem;">
        <li><b>Tek Kişilik / PvC:</b> 3, 4 veya 5 basamaklı sayıları tahmin etmeye çalışın. Bilgisayara karşı yarışabilirsiniz!</li>
        <li><b>Online Düello:</b> Gerçek rakiplerle eşleşin. Odayı kuran kuralları belirler (Süre, Hak Sayısı).</li>
        <li><b>Kariyer Yolu:</b> 5 farklı seviyeden oluşan zorlu bir yolculuk. Her seviyede zorluk artar ve ödül sandıkları açılır.</li>
    </ul>

    <h4 style="color:#8e44ad; margin-top:15px;">🅰️ Kelime Hazinesi</h4>
    <ul style="padding-left:20px; font-size:0.9rem;">
        <li><b>Kelime Avı:</b> 3 ile 10 harf arasındaki gizli kelimeleri bulun.</li>
        <li><b>Oyun Modları:</b> İster "Süreli" (3-7 dk) ister "Haklı" (15 hak) modunda oynayın.</li>
        <li><b>Kariyer (Kelime):</b> 8 seviyeli kelime kariyer haritası. Her 10 galibiyette seviye atlayın!</li>
        <li><b>İpuçları:</b> Takıldığınızda "Kahin" (Harf Aç) veya "Dondurucu" (Süre Durdur) kullanın.</li>
    </ul>

    <h4 style="color:#27ae60; margin-top:15px;">🐾 Pet (Evcil Hayvan) Sistemi</h4>
    <ul style="padding-left:20px; font-size:0.9rem;">
        <li><b>Sahiplenme:</b> Kedi, Köpek, Tilki, Panda gibi birçok 3D dostunuz sizi bekliyor.</li>
        <li><b>Besleme & Büyütme:</b> Petinizi besleyerek seviyesini yükseltin.</li>
        <li><b>Gezdirme:</b> Petiniz lobide sizinle birlikte gezer ve şans getirir!</li>
    </ul>

    <h4 style="color:#c0392b; margin-top:15px;">🤠 Ödül Avcıları & Klanlar</h4>
    <ul style="padding-left:20px; font-size:0.9rem;">
        <li><b>Ödül Avı:</b> Kendi belirlediğiniz zor bir sayıyla ilan verin veya başkalarının ilanlarını avlayarak büyük ödülü kapın!</li>
        <li><b>Klan Sistemi:</b> Kendi klanınızı kurun veya bir klana katılın. Birlikte sıralamada yükselin.</li>
    </ul>

    <h4 style="color:#2980b9; margin-top:15px;">💬 Sosyal & Profil</h4>
    <ul style="padding-left:20px; font-size:0.9rem;">
        <li><b>Özel Mesaj (DM):</b> Arkadaşlarınızla özel olarak mesajlaşın.</li>
        <li><b>Oda Sohbeti:</b> Oyun sırasında rakibinizle konuşun, emojiler gönderin.</li>
        <li><b>Gelişmiş Profil:</b> Biyografi, meslek, şehir ve avatarınızı düzenleyerek kendinizi tanıtın.</li>
    </ul>

    <h4 style="color:#f1c40f; margin-top:15px;">🏆 Diğer Özellikler</h4>
    <ul style="padding-left:20px; font-size:0.9rem;">
        <li><b>Günlük Görevler:</b> Her gün yenilenen görevleri yaparak altın kazanın.</li>
        <li><b>Piggy Bank (Kumbara):</b> Kazançlarınız kumbarada birikir.</li>
        <li><b>Market:</b> Kazandığınız altınlarla özel eşyalar ve güçlendirmeler satın alın.</li>
        <li><b>Temalar:</b> Karanlık mod, takım renkleri ve özel efektli temalarla oyunu kişiselleştirin.</li>
    </ul>

    <div style="margin-top:20px; text-align:center; font-size:0.8rem; color:#777;">
        <i>İyi Eğlenceler! - TahminSon Ekibi</i>
    </div>
</div>`;
        ...
        db.ref('settings/aboutText').once('value').then(snap => {
            ...
            if (container) {
                container.innerHTML = txt || DEFAULT_ABOUT_TEXT;
            }
            ...
            if (input && input.offsetParent !== null) { // Görünürse
                input.value = txt || DEFAULT_ABOUT_TEXT;
            }
        });
    }
    ...
    function recordIncomeForUser(username, amount) {
        const today = getDayKey();
        const week = getWeekKey();

        // Günlük Şampiyonluk
        db.ref(`daily_jackpot/${today}/${username}`).transaction(current => {
            return (current || 0) + amount;
        });

        // Haftalık Sıralama
        db.ref(`weekly_rankings/${week}/${username}`).transaction(current => {
            return (current || 0) + amount;
        });
    }
    ...
    function closePiggyAndReplay() {
        ...
        // Word Game Replay
        if (currentScreen === 'screen-word-game') {
             // Restart with last config
             // We stored config in wordGameState?
             // wordGameState has length and mode.

             // Re-run setup logic manually or extract `restartWordGame`?
             // Let's modify `startWordGameCustom` to optionally take params?
             // Or just set the DOM elements and call it?
             // But DOM might have changed.

             // Better: define `restartWordGame()` that uses `wordGameState` params.
             restartWordGame();
             return;
        }
    ...
    function submitWordGuess() {
        const guess = wordGameState.currentGuess;
        if (guess.length !== wordGameState.length) {
            return showToast(`Eksik harf! Kelime ${wordGameState.length} harfli olmalı.`);
        }
        ...
        // Check Win
        if (guess === wordGameState.secret) {
            wordGameState.active = false;
            setTimeout(() => {
                handleWordGameWin();
            }, 500);
        } else if (wordGameState.guesses.length >= wordGameState.maxGuesses) {
            wordGameState.active = false;
            AudioEngine.error();
            setTimeout(() => {
                alert(`MAALESEF BİLEMEDİN!\nDoğru Kelime: ${wordGameState.secret}`);
                changeScreen('screen-lobby');
                checkWordGameAd(); // Check ad on loss too? Usually yes.
            }, 500);
        }
        ...
        if (wordGameState.mode === 'pvc' && wordGameState.active) {
            setTimeout(() => {
                ...
                if (aiGuess === wordGameState.secret) {
                    wordGameState.active = false;
                    AudioEngine.error();
                    showToast("💻 Bilgisayar Kelimeyi Buldu: " + wordGameState.secret);
                    setTimeout(() => {
                    ...
                    showToast(`🤖 Bilgisayar Tahmini: ${aiGuess}`);
                }
            }, 1500);
        }
    }
    ...
    function renderWordBoard() {
        ...
        // Render past guesses
        wordGameState.guesses.forEach((guess, index) => {
            // Animasyon sadece son eklenen tahmin için çalışsın
            // (index === wordGameState.guesses.length - 1)
            // ANCAK: render her tuşta çalıştığı için bu da sürekli tetikler.
            // Çözüm: Animasyon sınıfını ayrı tutmak veya sadece yeni eklendiğinde DOM'a eklemek.
            // Basit çözüm: 'no-anim' sınıfı eklemek.

            // Biz burada basitçe, eğer bu tahmin "yeni" ise animasyonlu, eski ise animasyonsuz çizeceğiz.
            // Fakat DOM her render'da sıfırlanıyor.
            // Bu yüzden CSS'i değiştirelim: .correct, .present, .absent sınıfları varsayılan olarak animasyonsuz olsun.
            // Sadece .reveal sınıfı varsa animasyon oynasın.
            ...
            for (let i = 0; i < guess.length; i++) {
                ...
                // Sadece son tahmin ise 'reveal' ekle, ama bu da her harf yazışta render olduğu için sorun olabilir.
                // İdeal çözüm: submitWordGuess içinde animasyonu tetiklemek veya
                // render fonksiyonunun 'partial' çalışması.
                // Mevcut yapıyı bozmadan: Animasyon süresi 0.6s.
                // Eğer tahmin çok yeniyse (son 1 sn içinde eklendiyse?) animasyon koy.
                // Veya CSS'de animasyonu 'animation-iteration-count: 1' yaptık zaten.
                // Sorun DOM'un silinip tekrar eklenmesi.

                // HIZLI ÇÖZÜM: Animasyonu sadece "yeni" tahminlere verelim? Hayır, DOM yenileniyor.
                // ÇÖZÜM: Statik (bitmiş) satırlar için animasyonsuz sınıf kullanmak.

                let animClass = "";
                // Eğer bu son tahmin DEĞİLSE, animasyonu engelle
                if (index < wordGameState.guesses.length - 1) {
                    animClass = "no-anim";
                }

                cell.className = `word-cell ${status} ${animClass}`;
        ...
        if (wordGameState.active) {
            ...
            for (let i = 0; i < current.length; i++) {
                ...
                const cell = document.createElement('div');
                // Added 'active-row' class for visual highlight if needed
                cell.className = "word-cell active-row";
                if(letter !== ' ') {
    ...
    function startWordGameCustom() {
        ...
        const timeLimit = parseInt(document.getElementById('wordTimeSelect').value);
        const moveLimit = parseInt(document.getElementById('wordMoveSelect').value);
        ...
        if (!data) {
            return showToast("Bu seviye için kelime verisi bulunamadı!");
        }
        ...
        // Validation: Ensure secret length matches difficulty
        if (secret.length !== diff) {
            console.error("Data Mismatch: Secret " + secret + " length " + secret.length + " != diff " + diff);
            // Fallback: Pick another or show error
            // For now, trust WORD_DATA (which I fixed in previous step)
        }
        ...
        wordGameState = {
            ...
            length: diff,
            guesses: [],
            maxGuesses: moveLimit, // Use selected move limit
            currentGuess: "",
            level: lvl,
            startTime: Date.now(),
            timeLimit: timeLimit // Store time limit (minutes)
        };

        if(isPvC) {
            wordGameState.aiGuesses = [];
        }
        ...
        if (timeLimit > 0) {
            ...
            stopWordTimer(); // Clear any existing
            document.getElementById('wordTimerDisplay').style.display = 'none';
        }

        const modeText = isPvC ? '🆚 Bilgisayar' : 'Tek Kişilik';
        document.getElementById('wordLevelDisplay').innerText = `${diff} Harf - ${moveLimit} Hamle - ${modeText}`;
    }
    ...
    function createWordRoom() {
        ...
        // Pick Secret Word (Shared? or Each has own?)
        // "Rakam oyununda olan tüm özellikler" -> Usually shared secret or symmetrical.
        // Let's pick ONE secret word for both to guess. Race.
    ...
    function joinWordRoom(c) {
        ...
        db.ref('rooms/' + c).once('value').then(s => {
            const r = s.val();
            if (!r || r.type !== 'word') return showToast("Oda Yok/Hatalı");
    ...
    function listenWordRoom(c) {
        db.ref('rooms/' + c).on('value', s => {
            ...
            if (r.status === 'playing' && currentScreen !== 'screen-word-game') {
                ...
                wordGameState = {
                    ...
                    maxGuesses: 6,
                    currentGuess: "",
                    level: r.config.length - 2
                };
                changeScreen('screen-word-game');
                renderWordBoard();
                updateWordKeyboard();
                showToast("DÜELLO BAŞLADI!");
            }
            ...
            if (r.winner) {
                if (r.winner === user.name) {
                    ...
                    alert("KAYBETTİN! Kazanan: " + r.winner);
                    changeScreen('screen-lobby');
                }
                db.ref('rooms/' + c).off();
            }
        ...
        // Listen for Opponent Moves (Optional: Show ghost rows?)
        // For now, simpler: First to guess wins.

        db.ref('rooms/' + c + '/word_moves').on('child_added', s => {
            const m = s.val();
            if (m.g === wordGameState.secret) {
                // Someone won
                db.ref('rooms/' + c).update({ winner: m.u });
            }
        });
    }
    ...
    function openWordCareer() {
        ...
        db.ref('users').once('value')
            ...
            .catch(error => {
                console.error("Kelime Harita Hatası:", error);
...
function restartWordGame() {
        ...
        renderWordBoard();
        updateWordKeyboard();
        closeModal('modal-end'); // Just in case
    }
    ...
    function drawWordMapWithUsers(container, allUsers) {
        ...
        // User asked: "her 10 oyunda bir seviye atlansın".
    ...
    function claimWordCareerReward(lvl) {
        ...
        if (claimedRewards[lvl]) return showToast("Bu ödülü zaten aldın!");
        const canClaim = (lvl < currentLvl) || (lvl === currentLvl && currentWins >= 10);
        if (!canClaim) return showToast("Ödülü almak için bu seviyeyi tamamlamalısın!");
    ...
    function startWordCareerLevel(lvl) {
        if(user.wordLevel < lvl) return showToast("Önceki seviyeyi tamamla!");
        ...
        // Setup Ads? "4 oyunda bir geçiş reklamı"
        handleWordGameAd();
    ...
    function handleWordGameAd() {
        wordGameAdCounter++;
        // NATIVE ENTEGRASYON İÇİN SADECE LOG
        /*
        ...
        */
            // User said "4 oyunda bir geçiş reklamı olsun". Usually at end or start.
            // Start is annoying. Let's trigger it at end?
            // Or just trigger logic here but show at end?
            // Better: Trigger at END of game.
            // I'll move logic to game end.
        }
    }
            ...
            if (wordGameState.level === user.wordLevel && user.wordLevel <= 8) {
                ...
                if (user.wordCareerWins >= 10) {
                    if (user.wordLevel < 8) {
                        user.wordLevel++;
                        user.wordCareerWins = 0;
                        showToast(`🎉 SEVİYE ATLADIN! Yeni Seviye: ${user.wordLevel}`);
                    } else {
                        showToast("🏆 KARİYER TAMAMLANDI! EFSANESİN!");
                    }
                } else {
                    showToast(`✅ İlerleme: ${user.wordCareerWins}/10`);
                }
    ...
    function sendWordInvite(targetUser) {
        if (user.gold < 50) return showToast("50 Altın Gerek (Oda Kurmak İçin)");
        ...
        closeModal('modal-word-setup');
        showToast(`📨 ${targetUser} davet edildi!`);
    ...
    function listenForInvites() {
        ...
        inviteRef.on('child_added', snapshot => {
            ...
            document.getElementById('inviterNameDisplay').innerText = invite.from;
            const typeText = (invite.type === 'word') ? "KELİME DÜELLOSU" : "Sayı Tahmin";
            document.querySelector('#modal-invite-incoming h3').innerText = `📨 DAVET VAR! (${typeText})`;
    ...
    // OVERRIDE AD MANAGER MOCK
    if (typeof AdManager !== 'undefined') {
        AdManager.runWebMock = function (isInterstitial = false) {
            if (this.mockRunning) return;
            this.mockRunning = true;
            ...
            // Show Loading / Count
            let timeLeft = 3;
            modalBody.innerHTML = `<h3>📺 Reklam</h3><p style="font-size:1.2rem; font-weight:bold;">${timeLeft}</p>`;
            ...
            const interval = setInterval(() => {
                ...
                if(timeLeft > 0) {
                    ...
                    clearInterval(interval);
                    // Time Over: Show Close Button
                    modalBody.innerHTML = `
                        <h3>✅ Reklam Bitti</h3>
                        <p>Teşekkürler!</p>
                        <button class="btn-red" onclick="AdManager.closeWebMock(${isInterstitial})">KAPAT (X)</button>
                    `;
                }
            }, 1000);
        };

        AdManager.closeWebMock = function(isInterstitial) {
             const modalBody = document.querySelector('#modal-ad-watch .modal-box');
             if(modalBody && this.originalMockContent) modalBody.innerHTML = this.originalMockContent;
